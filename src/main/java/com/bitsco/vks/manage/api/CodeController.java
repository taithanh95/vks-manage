...
import com.bitsco.vks.manage.service.CodeService;
...
public class CodeController {
    ...
    @CrossOrigin(origins = "*")
    @PostMapping("/getList/")
    public ResponseEntity<?> findByStatus(@RequestBody Code code) throws Exception {
        try {
            return new ResponseEntity<ResponseBody>(new ResponseBody(Response.SUCCESS, codeService.getList(code)), HttpStatus.OK);
    ...
    public ResponseEntity<?> getPage(@RequestBody PageRequest pageRequest) throws Exception {
        try {
            return new ResponseEntity<>(new com.bitsco.vks.common.response.ResponseBody(Response.SUCCESS, codeService.getPage(pageRequest)), HttpStatus.OK);
        } catch (CommonException e) {
            ...
            LOGGER.error("Exception when /code/getPage/ ", e);
